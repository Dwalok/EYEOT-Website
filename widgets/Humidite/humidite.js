(function() {
    ...
    let wrapper = (function() {
        ...
        return document.getElementById('widget-humidity-standalone')
            || document.querySelector('.widget.widget-humidite')
            || null;
    })();
    ...
    const valeurEl = wrapper.querySelector('.widget-valeur');
    const humiditeFill = wrapper.querySelector('.hygro-fill');
    const sourceEls = wrapper.querySelectorAll('.widget-source');
    const graphCanvas = wrapper.querySelector('.humidite-graph');
    ...
    window[widgetId] = {
        updateValue: function(valeur, timestamp) {
            console.log('updateValue humidite', valeur, timestamp, widgetId);
            ...
            if (humiditeFill && Number.isFinite(numVal)) {
                const percentage = Math.min(Math.max(numVal, 0), 100);
                humiditeFill.style.width = `${percentage}%`;

                if (numVal < 30) {
                    humiditeFill.style.backgroundColor = '#D0021B';
                } else if (numVal < 60) {
                    humiditeFill.style.backgroundColor = '#7ED321';
                } else {
                    humiditeFill.style.backgroundColor = '#4A90E2';
                }
            }
