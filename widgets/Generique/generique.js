(function() {
    ...
    let wrapper = (function() {
        ...
        return document.getElementById('widget-generique-standalone')
            || document.querySelector('.widget.widget-generique')
            || null;
    })();

    if (!wrapper) {
        console.error('Widget generique : wrapper introuvable');
        return;
    }
    ...
    const valeurEl = wrapper.querySelector('#valeur-generique');
    const sourceEls = wrapper.querySelectorAll('.widget-source');
    const graphCanvas = wrapper.querySelector('.generique-graph');
    ...
    window[widgetId] = {
        updateValue: function(valeur, timestamp) {
            console.log('updateValue generique', valeur, timestamp, widgetId);
            ...
            if (timestamp) {
                const sourceEl = wrapper.querySelector('#source-generique');
                if (sourceEl) {
                    sourceEl.title = 'Derniere mise a jour : ' + timestamp;
                }
            }

            addPoint(valeur, timestamp);
        }
    };
