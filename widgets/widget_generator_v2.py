...
# ---------- CSS générique (vertical + horizontal style humidité) ----------
...
/* Rangée pastilles + infos (comme humidité) */
.row-split {
    display: flex;
    align-items: center;
    gap: 10px;
...
/* Jauge horizontale fine façon hygromètre */
.hygro-container {
    height: 5px;
    width: 100%;
...
def build_html(display_name: str,
    ...
    - vertical : style Température (widget-split + gauge-container vertical)
    - horizontal : style Humidité (widget-content-vertical + row-split + hygro-container)
    """
...
"""
        ...
        # ==== VERSION STYLE HUMIDITÉ (BARRE HORIZONTALE) ====
        html = f"""<link rel="stylesheet" href="/static/widgets/{widget_folder}/{base_name}.css">
...
{dots_html if has_dots else ""}
    ...
    <div class="hygro-container">
      <div class="hygro-fill"></div>
    </div>
...
def build_js(slug: str,
             ...
             orientation: str) -> str:
    """
    JS du widget, compatible avec dashboard.js :
    - window[widgetId].updateValue(valeur, timestamp)
    - vertical : gauge-fill en hauteur
    - horizontal : hygro-fill en largeur (style humidité)
    """
    ...
    if orientation == "vertical":
        ...
        gauge_code = """            const gaugeFill = wrapper.querySelector('.hygro-fill');
            if (gaugeFill) {
                const percentage = Math.min(Math.max(valeur, 0), 100);
                gaugeFill.style.width = `${percentage}%`;

                // palette type humidité
                if (valeur < 30) {
                    gaugeFill.style.backgroundColor = '#D0021B';
                } else if (valeur < 60) {
                    gaugeFill.style.backgroundColor = '#7ED321';
    ...
    js = f"""(function() {{
    // --- Récupération robuste du wrapper ---
...
class WidgetGeneratorWindow(QMainWindow):
    ...
    def generate_files(self):
        try:
            ...
            if not unit:
                # unité par défaut selon le type
