(function () {
  ...
  let wrapper = (function() {
    ...
    return document.getElementById('widget-luminosite-standalone')
      || document.querySelector('.widget.widget-luminosite')
      || null;
  })();
  ...
  const valeurEl = wrapper.querySelector('#valeur-luminosite');
  const luminoFill = wrapper.querySelector('#lumino-fill');
  const sourceEls = wrapper.querySelectorAll('.widget-source');
  const graphCanvas = wrapper.querySelector('.luminosite-graph');
  ...
  function drawGraph() {
    ...
    for (let i = 0; i <= stepsY; i++) {
      ...
      ctx.stroke();
      ctx.fillStyle = 'rgba(255,255,255,0.7)';
      ctx.fillText(`${val.toFixed(1)} lux`, pad.left - 6, y);
    }
  ...
  window[widgetId] = {
    updateValue: function (valeur, timestamp) {
      console.log('updateValue luminosite', valeur, timestamp, widgetId);

      const clamped = Math.min(Math.max(Number(valeur) || 0, 0), 1000);
      const percentage = (clamped / 1000) * 100;

      if (valeurEl) {
        valeurEl.innerHTML = `${clamped.toFixed(2)} <span class="unite">lux</span>`;
      }

      if (luminoFill) {
        luminoFill.style.width = `${percentage}%`;

        if (clamped < 100) {
          luminoFill.style.background = 'linear-gradient(to right, #4A90E2, #50C8FF)';
        } else if (clamped < 500) {
          luminoFill.style.background = 'linear-gradient(to right, #F5A623, #F8E71C)';
        } else {
          luminoFill.style.background = 'linear-gradient(to right, #FFD27A, #FF9E3B)';
        }
      }

      addPoint(clamped, timestamp);
    }
  };
